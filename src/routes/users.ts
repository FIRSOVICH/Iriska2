...
router.get('/me', auth, async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await query(
      `SELECT u.id, u.name, u.username, u.email, u.avatar_url, u.created_at,
...
router.patch('/me', auth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (bio !== undefined) {
      await query(
        'INSERT INTO user_profiles (user_id, bio) VALUES ($1,$2) ON CONFLICT (user_id) DO UPDATE SET bio=$2, updated_at=NOW()',
        [req.userId, bio]
      );
    }
    const { rows } = await query('SELECT id,name,username,email,avatar_url FROM users WHERE id=$1', [req.userId]);
    res.json(rows[0]);
...
router.get('/:id', auth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const user = rows[0];
    if (user.hide_online) { delete user.status_preset; }
    res.json(user);
  } catch (e) { res.status(500).json({ error: 'Ошибка' }); }
});

// GET /api/users/me/contacts
router.get('/me/contacts', auth, async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await query(
      `SELECT c.id, c.nickname, c.blocked, u.id as user_id, u.name, u.username, u.avatar_url,
...
router.post('/me/contacts', auth, async (req: AuthRequest, res: Response) => {
  const { contactId, nickname } = req.body;
  if (!contactId) return res.status(400).json({ error: 'contactId обязателен' });
  try {
    await query(
      'INSERT INTO contacts (user_id, contact_id, nickname) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING',
...
router.delete('/me/contacts/:id', auth, async (req: AuthRequest, res: Response) => {
